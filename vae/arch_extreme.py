...
class VAE():
    ...
    def _build(self):
        ...
        def vae_r_loss(y_true, y_pred):
            ...
            reconstruction_loss = 64 * 64 * 3 * mse(flat_true, flat_pred)

            imp_indices = K.cast(flat_true > 0.95, dtype='float32')
            nb_imp = K.sum(imp_indices)

            imp_true = flat_true * imp_indices
            imp_pred = flat_pred * imp_indices
            reconstruction_loss += nb_imp * 100 * mse(imp_true, imp_pred)
