...
from world_model.rnn.arch_v4 import RNN
import argparse
import numpy as np
import os
...
def main(args):
	...
	for step in range(steps):
		...
		cumrew = np.cumsum(rew, axis=1)
		print("shapes are: z={}, action={}, reward={}, cumrew: {}, done={}".format(np.shape(z),np.shape(action),np.shape(rew),np.shape(cumrew), np.shape(done)))
		print("rnnin shapes are: z={}, action={}, reward={}, done={}".format(np.shape(z[:, :-1,:]),np.shape(action[:, :-1,:]),np.shape(rew[:, :-1,:]),np.shape(cumrew[:,:-1,:]),np.shape(done[:, :-1,:])))
		rnn_input = np.concatenate([z[:, :-1, :], action[:, :-1, :], rew[:, :-1, :], cumrew[:,:-1,:]], axis = 2)
		rnn_output = np.concatenate([z[:, 1:, :], action[:, 1:,:], rew[:, 1:, :], cumrew[:,1:,:], done[:, 1:,:]], axis = 2) # ,action[:, 1:], done[:, 1:]
		...
		vcumrew = np.cumsum(vrew, axis=1)
		print("shapes are: z={}, action={}, reward={}, cumrew={}, done={}".format(np.shape(vz),np.shape(vaction),np.shape(vrew),np.shape(vcumrew),np.shape(vdone)))
		vrnn_input = np.concatenate([vz[:, :-1, :], vaction[:, :-1, :], vrew[:, :-1, :], vcumrew[:, :-1, :]], axis = 2)
		vrnn_output = np.concatenate([vz[:, 1:, :], vaction[:, 1:,:], vrew[:, 1:, :], vcumrew[:, :-1, :], vdone[:, 1:,:]], axis = 2) # ,action[:, 1:], done[:, 1:]
